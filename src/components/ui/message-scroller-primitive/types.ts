...
// Default scrollMargin for programmatic targets.
...
// Viewport alignment for programmatic jumps.
type MessageScrollerScrollAlign = "start" | "center" | "end" | "nearest";

// Options for scrollToEnd and scrollToStart.
...
// Scroll snapshot for which edges the viewport can still scroll toward.
...
type MessageScrollerProviderProps = {
	...
	// Default margin on the aligned edge for commands. Defaults to 0.
	scrollMargin?: number;
};
...
type MessageScrollerItemProps = React.ComponentProps<"div"> & {
	// Stable row id for prepend preservation.
	messageId?: string;
...
// Minimal external store backing the scroll button state.
